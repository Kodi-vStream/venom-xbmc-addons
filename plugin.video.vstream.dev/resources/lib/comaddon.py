...
class addon(xbmcaddon.Addon):
    ...
    #def __init__(self, id='plugin.video.vstream.dev'):
    #    xbmcaddon.__init__(id)
    #    pass

    def VSlang(self, lang):
        return xbmc.translatePath(self.getLocalizedString(lang))
        #xbmcaddon.Addon('plugin.video.vstream.dev').getLocalizedString(lang))
        #Bug avec accent xbmc.translatePath(xbmcaddon.Addon('plugin.video.vstream.dev').getLocalizedString(lang)).decode("utf-8")
...
def VSlog(e, level=xbmc.LOGDEBUG):
    ...
    if (addon('plugin.video.vstream.dev').getSetting('debug') == 'true'):
...
def VSread(sHtmlContent):
    import xbmcvfs
    file = "special://userdata/addon_data/plugin.video.vstream.dev/html.txt"
