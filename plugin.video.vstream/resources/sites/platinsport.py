...
def showMovies3():  # affiche les videos disponible du live
    ...
    #sPattern = "ace(.*?)sream(.*?)$"
    #sPattern = "\\bacestream\w+\\b"
    #sPattern = "\[(.*?)[acestream://(.*?)]+[<]*"

    oParser = cParser()

    #VSlog(SITE_NAME + ' - showMovies3 - sHtmlContent ' + sHtmlContent)

    #aResult = oParser.parse(sHtmlContent, sPattern)
    aMatches = re.compile(sPattern, re.MULTILINE).findall(sHtmlContent)
    ...
    if len(aMatches)<1:
        ...
        for aEntry in aMatches:
            ...
            VSlog(SITE_NAME + ' - showMovies3 - ' + ' sUrl4=' + sUrl4 + '. ')
...
def showHosters():  # affiche les videos disponible du live
    ...
    if streamurl.count('sopcast') >0:
        ...
        lien = 'XBMC.RunPlugin(plugin://program.plexus/?url='+streamurl+'&mode=2&name=Vstream)'
    ...
    if streamurl.count('acestream') > 0:
        ...
        lien = 'XBMC.RunPlugin(plugin://program.plexus/?url='+streamurl+'&mode=1&name=Vstream)'
        xbmc.executebuiltin( lien )
